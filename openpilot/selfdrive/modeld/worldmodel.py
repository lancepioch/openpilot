...
class WorldModel:
  ...
  def attention(self, x, name, layer, start_frame, last_frame=False):
    ...
    for start in range(query_start, seq, query_chunk):
      ...
      if x.dtype == dtypes.bfloat16 and width // heads == 64 and getattr(Device[x.device], "arch", "") in {"gfx1200", "gfx1201"}:
        from openpilot.selfdrive.modeld.worldmodel_kernels import attention_softmax
    ...
    y = chunks[0].cat(*chunks[1:], dim=2).transpose(1, 2).reshape(batch, seq - query_start, width)
